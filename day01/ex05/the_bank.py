class Account(object):
	ID_COUNT = 1

	def __init__(self, name, **kwargs):
		self.id = self.ID_COUNT
		self.name = name
		self.__dict__.update(kwargs)
		if hasattr(self, 'value'):
			self.value = 0
		Account.ID_COUNT += 1
	
	def transfer(self, amount):
		self.value += amount

class Bank(object):
	"""The bank"""
	def __init__(self):
		self.account = []

	def add(self, account):
		self.account.append(account)
	
	def transfer(self, origin, dest, amount):
		if origin not in self.account or dest not in self.account:
			print('Invalid transaction')
		elif origin.value < amount:
			print('Not enough funds in your account :(')
		else:
			origin.value -= amount
			dest.value += amount
			print('The transaction was succesfully accomplished')

	def fix_account(self, account):
		pass

myBank = Bank()
myAccount = Account('Eudald', value = 0)
charlesAccount = Account('Charles', value = 0)
x = 1

myBank.add(myAccount)
myBank.add(charlesAccount)
myBank.transfer(myAccount, charlesAccount, 1000)
myAccount.value = 1000
myBank.transfer(myAccount, charlesAccount, 1000)

print(myAccount.value)